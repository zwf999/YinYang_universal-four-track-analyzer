...
class BidirectionalAnalysisFixed:
    def __init__(self):
        self.min_samples = 3
    ...
    def bidirectional_tse(self, values):
        ...
        正向：原始顺序计算TSE
        反向：反转数值顺序后计算TSE（模拟时间倒流）
        """
        if len(values) < 3:
            return None, None, None
        
        # 正向TSE：原始顺序
        binary_forward = self.data_to_binary(values)
        tse_forward = self.calculate_tse(binary_forward)
        
        # 反向TSE：反转数值顺序（模拟时间倒流）
        values_reversed = values[::-1]  # 反转列表
        binary_reverse = self.data_to_binary(values_reversed)
        tse_reverse = self.calculate_tse(binary_reverse)
    ...
    def sliding_window_bidirectional(self, data, window_size=50):
        """滑动窗口双向分析"""
            ...
            if len(depths) >= 3:
                ...
                if tse_fwd is not None and tse_fwd < 100:
                    results.append({
                        ...
                        'tse_reverse': tse_rev,
                        'delta_tse': delta_tse,
                        'has_direction': delta_tse > 0.1,  # 方向性阈值
                        'count': len(depths)
                    })
        ...
        if len(pre_shock) >= 30:
            ...
            if len(depths) >= 3:
                ...
                results['pre_shock'] = {
                    ...
                    'tse_reverse': tse_rev,
                    'delta_tse': delta,
                    'has_direction': delta > 0.1 if delta else False
                }
    ...
    def run_bidirectional_analysis(self):
        """运行双向比较分析"""
        print("=" * 70)
        print("太仪v2.0 - 双向比较分析（修正版）")
        print("=" * 70)
        print("正向时间序列 vs 反向时间序列 → ΔTSE（方向性指标）")
        print("=" * 70)
        ...
        # 保存结果
        output_file = "data2/results/bidirectional_analysis_fixed.csv"
...
if __name__ == "__main__":
    analysis = BidirectionalAnalysisFixed()
