...
def analyze_dna_file(file_path):
    """
    分析DNA文件
    ...
    try:
        ...
        # 分析编码后的数字序列
        print("\n正在分析编码后的序列...")
        result = analyzer.analyze(encoded_digits[:100000])  # 只分析前10万位，避免计算量过大
...
def display_analysis_result(result):
    ...
    if 'statistical' in result:
        ...
        # 数字分布
        if 'digit_distribution' in stats:
            print("  数字分布:")
            distribution = stats['digit_distribution']
            for digit in range(10):
                freq = distribution.get(digit, 0)
                print(f"    {digit}: {freq}")
        ...
        for i in range(1, 5):
            ...
            if track:
                ...
                if 'forward' in track:
                    ...
                    if 'symbol_pairs' in forward:
                        pair_ratio = forward['symbol_pairs'].get('pair_ratio', 0)
                        print(f"    正向配对率: {pair_ratio:.4f}")
                if 'backward' in track:
                    backward = track['backward']
                    if 'symbol_pairs' in backward:
                        pair_ratio = backward['symbol_pairs'].get('pair_ratio', 0)
                        print(f"    反向配对率: {pair_ratio:.4f}")
                if 'symmetry' in track:
                    symmetry = track['symmetry'].get('overall_symmetry', 0)
                    print(f"    对称性: {symmetry:.4f}")
    
    # 模式分析
    if 'pattern' in result:
        pattern = result['pattern']
        print("\n模式分析:")
        print(f"  总模式数: {pattern.get('total_patterns', 0)}")
        print(f"  模式密度: {pattern.get('pattern_density', 0):.4f}")
...
def analyze_all_dna_files(directory):
    """
    分析目录中的所有DNA文件
    ...
    print("="*60)
    healthy_results = []
    for file in healthy_files[:2]:  # 只分析前2个，避免计算量过大
        result = analyze_dna_file(file)
        if result:
    ...
    print("="*60)
    cancer_results = []
    for file in cancer_files[:2]:  # 只分析前2个，避免计算量过大
        result = analyze_dna_file(file)
        if result:
